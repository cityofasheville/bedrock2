...
async function readEtlList(client, run_groups) {
  ...
  if (run_groups.length > 0) {
    ...
    const sql = `SELECT * FROM bedrockprod.etl where run_group in (${rgString}) and active = true;`;
...
async function readDependencies(client, assetMap) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const asset = arr[i][1];
    const sql = `SELECT * FROM bedrockprod.dependencies where asset_name = '${arr[i][0]}';`;
    ...
    for (let j = 0; j < res.rowCount; j += 1) {
      ...
      if (d[0] === '#') { // Aggregate Dependancy. Look up list of dependencies in tags
        ...
        const aggrSql = `SELECT asset_name FROM bedrockprod.asset_tags where tag_name = '${aggrDependency}';`;
        // eslint-disable-next-line no-await-in-loop
...
async function readLocationFromAsset(client, assetName) {
  ...
  const sql = `SELECT location::jsonb || ('{"asset":"' || asset_name || '"}')::jsonb as location
              FROM bedrockprod.assets where asset_name = '${assetName}';`;
  // eslint-disable-next-line no-await-in-loop
  const res = await client.query(sql)
    .catch((err) => {
      const errmsg = pgErrorCodes[err.code];
...
async function readAggregateData(client, tempLocation, location, taskSource) {
  ...
  location->>'tab' tab from bedrockprod.assets a
  inner join bedrockprod.asset_tags using (asset_name)
...
async function readTasks(client, assetMap) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    ...
    const asset = arr[i][1];
    const sql = `SELECT * FROM bedrockprod.tasks where asset_name = '${assetName}' order by seq_number;`;
...
async function getRunGroups(client, debug) {
  const sql = 'SELECT run_group_name, cron_string FROM bedrockprod.run_groups;';
  return client.query(sql)
    .then((res) => {
      const run_groups = [];
      for (let i = 0; i < res.rowCount; i += 1) {
...
async function verifyAssetExists(client, assetName) {
  const sql = `SELECT * FROM bedrockprod.etl where asset_name = '${assetName}';`;
  return client.query(sql)
    .then((res) => {
      if (res.rowCount === 0) {
        return false;
