...
exports.lambda_handler = function(event, context) {
    const task = new Promise(async ( resolve ) => { 
        ...
        try{
            ...
            const since_data = { 
                "num_weeks": etl.num_weeks,
                "column_to_filter": etl.column_to_filter
            }
            if (!etl.active) {
                ...
                for (const eachloc of bothloc) {
                    ...
                    if(eachloc.location.conn_info.type === 'postgresql') {
                        streams[eachloc.name] = await get_pg_stream(eachloc.location,since_data)
                    }else if(eachloc.location.conn_info.type === 'sqlserver') {
                        streams[eachloc.name] = await get_ss_stream(eachloc.location,since_data)
                    }else if(eachloc.location.conn_info.type === 'google_sheets') {
                        throw ("Google Sheets 'table_copy_since' not implemented")
                    }
                }
