...
*/
function get_pg_stream(location,since_data) {
    return new Promise(async function(resolve, reject) {
        ...
        try{
            ...
            let copy_from_temp = () => {
                ...
                BEGIN TRANSACTION;
                ${serial_to_append}
                DELETE FROM ${tablename} WHERE ${since_data.column_to_filter} >= NOW() - interval '${since_data.num_weeks} WEEK;' ;
            ...
            if(location.fromto == 'source_location') {
                let query_string = `COPY (SELECT * FROM ${tablename} 
                    WHERE ${since_data.column_to_filter} >= NOW() - interval '${since_data.num_weeks} WEEK;') TO STDOUT WITH (FORMAT csv);`
